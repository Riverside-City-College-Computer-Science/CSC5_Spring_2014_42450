...
//Global Constants
const int COLS=2;//Years and Savings
...
void filRetr(float retre[][COLS],int rSize, float mb, float yDep, float salary){
    //Calculate the retirement savings
...
void prntRetr(float retre[][COLS],int rSize){
    ...
    for(int year=1;year<=rSize;year++){
        cout<<setw(4)<<static_cast<int>(retre[year-1][0])<<setw(10)
                <<retre[year-1][1]<<endl;
    }
}
//Function Retirement
