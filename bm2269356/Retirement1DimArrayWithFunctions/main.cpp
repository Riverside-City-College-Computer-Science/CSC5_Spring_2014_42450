...
void filRetr(float retrmnt[], int size, float muniBnd, float yrlyDep, float salary) {
        for(int year=1;year<=size;year++) {
        retrmnt[year-1] = retire(0,muniBnd,year,yrlyDep*salary);
    }
}

void prntRetr(float retire[],int size) {
    for(int year=1;year<=size;year++) {
        cout << setw(10) << left << year << right << " $" << retire[year-1] << endl;
    }
}