...
void filRetr(float retrmnt[],int years[],int rSize
             ,float mB,float yDep,float salary){
    //Calculate for variables
    for(int year=1;year<=rSize;year++){
        retrmnt[year-1]=year;
        retrmnt[year-1]=(0,mB,year,yDep*salary);
        
    }
}
void prntRetr(float years [],int years[],int rSize){
        //Print 
        cout<<setprecision(2)<<fixed<<showpoint;
        cout<<"YEAR SAVINGS $ "<<endl;
        for(int year=1;year<=rSize;year++){
            cout<<setw(4)<<static_cast<int>(years[year-1][0]
                    <<setw(10)<<years[year-1][1]<<endl;
        }
}
//Function Retirement
//Inputs
