...
void filRetr(float retrmnt[],int rSize
             ,float mB,float yDep,float salary){
    //Calculate for variables
    for(int year=1;year<=rSize;year++){
        retrmnt[year-1]=retire(0,mB,year,yDep*salary);
    }
}
void prntRetr(float [],int){
        //Print 
        cout<<"YEAR SAVINGS $ "<<endl;
        or(int year=1;year<=rSize;year++){
            cout<<setw(4)<<year,,setw(10<<retr[year-1]<<endl;
        }
}
//Function Retirement
//Inputs
